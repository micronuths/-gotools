...
func New(writer io.Writer) *GinkgoReporter {
	logger := config.NewLogger("ginkgo")
	logger.RegisterSink(config.NewWriterSink("", writer, config.DEBUG))
...
func (g *GinkgoReporter) SpecSuiteWillBegin(config ginkgoConfig.GinkgoConfigType, summary *types.SuiteSummary) {
...
func (g *GinkgoReporter) SpecWillRun(specSummary *types.SpecSummary) {
	g.wrappedWithNewlines(func() {
		...
		g.session.Info("start", config.Data{
			"summary": SpecSummary{
...
func (g *GinkgoReporter) SpecDidComplete(specSummary *types.SpecSummary) {
	g.wrappedWithNewlines(func() {
		...
		if passed(specSummary.State) {
			g.session.Info("end", config.Data{
				"summary": summary,
			})
		} else {
			summary.StackTrace = specSummary.Failure.Location.FullStackTrace
			g.session.Error("end", errorForFailure(specSummary.Failure), config.Data{
				"summary": summary,
			})
		}
		g.session = nil
